...
struct SharedState;   // forward declaration so NextInst's tag has file scope

typedef void (*NextInst)(struct SharedState*, struct ElevatorNode*);
...
typedef struct ElevatorNode {
    ...
    uint32_t id;                 // User -- for trace printing ("User <id>"), set once at U1
} ElevatorNode;
