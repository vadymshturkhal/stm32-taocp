...
* **Base case: `n = 50, input_pairs_len = 139 (Hamiltonian Path with 90 cache-thrashing jump edges)`:**
    ...
    * **`Queue based` with `Sequential Memory Allocation`:**
        ...
        * **Additional `n = 500, input_pairs_len = 1000` case stats:**
            ...
            * **Summary:** Compiler scaling behavior flips drastically at scale. Clang beats Rust at a small dataset `n=50` but suffers heavily at `n=500`, proving its aggressive loop unrolling `(736 bytes)` chokes at scale, likely due to I-Cache pressure. Rust scales beautifully thanks to strict aliasing guarantees. Handwritten ASM is the ultimate winner and its microscopic `280-byte` footprint easily fits in the cache, allowing it to scale far more efficiently (5.5x growth) than Clang (8.4x growth) on the exact same dataset
