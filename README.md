...
* **Base case: `n = 50, input_pairs_len = 139 (Hamiltonian Path with 90 cache-thrashing jump edges)`:**
    ...
    * **`Queue based` with `Sequential Memory Allocation`:**
        ...
        * **Additional `n = 500, input_pairs_len = 1000` case stats:**
            ...
            * **Summary:** Clang beats Rust at `n=50` but loses at `n=500`. GCC is slowest at both but scales most predictably. ASM wins at both and scales identically to Clang — the unrolling that bloats Clang doesn't help it at scale.
