...
// Prototypes
uint8_t c_algorithm_t_sequential(uint8_t n, Pair* input_pairs, uint8_t input_pairs_len, uint32_t* output);
...
void comparing_topological_sort_sequential(void) {
	...
	start = DWT->CYCCNT;
	gcc_topological_status = c_algorithm_t_sequential(n, input_pairs, input_pairs_len, output);
	if (gcc_topological_status == 0) return 0;
	end = DWT->CYCCNT;
